...
// ========== HELPER ==========
/**
 * Tính kết quả dựa trên điểm lần 1 và lần 2
 * - Lần 1 >= 5 → Đạt luôn
 * - Lần 1 < 5, chưa có lần 2 → Chờ điểm lần 2
 * - Lần 1 < 5, lần 2 >= 5 → Đạt
 * - Lần 1 < 5, lần 2 < 5 → Trượt
 */
...
export default function ScoreModal({ editItem, onClose, onSave, existingScores = [] }) {
    ...
    const {
        ...
        onFieldsChange: (name, value, currentForm, deps) => {
            ...
            //     // // Auto-fill courseId + courseName
            //     // updatedForm.courseId = student.courseId;
            //     // updatedForm.courseName = course?.courseName || '';
            //     // Lọc class student đã đăng ký → lưu vào classOptions để classPicker dùng
            //     updatedForm.classOptions = deps.classes.filter(cl => registeredClassId.includes(cl.id));
            //     // Reset classId vì student mới
            //     updatedForm.classId = '';
            //     updatedForm.courseId = '';   // ❗ reset
            //     updatedForm.courseName = ''; // ❗ reset
            //     return updatedForm
            // }
            if (name === 'studentId') {
                const student = deps.students.find(s => Number(s.id) === Number(value));
                ...
                const registeredClassId = studentClassRef.current
                    .filter(sc => Number(sc.studentId) === Number(student.id))
                    .map(sc => sc.classId);
                // ... giữ nguyên 2 khối alertError phía trên ...
